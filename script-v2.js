(() => {
  // Retire les éléments ajoutés par une éventuelle ancienne version mise en cache.
  document.querySelectorAll('head > style').forEach((style) => style.remove());
  document.querySelectorAll('.hj-assistant, .assistant-panel, .assistant-launcher, .availability-note, .button-cv').forEach((element) => element.remove());
